...
function Drawing() {
  ...
  const [drawingposts, setDrawingposts] = useState([]);
...
const handleDelete = (postId) => {
  ...
  useEffect(() => {
    artistAxiosInstance
      .get("/posts?category=Drawing", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      .then((response) => {
        setDrawingposts(response.data.categoryPosts);
        setLoading(false);
      })
      .catch((error) => {
